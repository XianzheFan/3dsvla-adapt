...
def masked_image(mask, image, out_dir, rgb_id, category):
    ...
    if len(mask_reshape.shape) == 2:
        ...
        box_mask(new_image_pil,os.path.join(out_dir,rgb_id+'_{}_{}_box_v1_1022.png'.format(0,category)),0)
        new_image_pil2.save(os.path.join(out_dir,rgb_id+'_{}_{}_segmask_v1_1022.png'.format(0,category)))
    else:
        for i in range(mask.squeeze().squeeze().cpu().numpy().shape[0]):
            ...
            new_image_pil = Image.fromarray(new_image)
            new_image_pil2 = new_image_pil
            box_mask(new_image_pil,os.path.join(out_dir,rgb_id+'_{}_{}_box_v1_1022.png'.format(i,category)),i)
            
            new_image_pil2.save(os.path.join(out_dir,rgb_id+'_{}_{}_segmask_v1_1022.png'.format(i,category)))
...
def generate_mask(img_path, model, out_dir, rgb_id, category):
    ...
    if 'knife' in category:
        ...
        for i,item in enumerate(pred_phrases):
            # print(i,item)
            if abs(boxes_filt[i][3]-boxes_filt[i][1])>abs(boxes_filt[i][2]-boxes_filt[i][0]) and boxes_filt[i][1]>10 and abs(boxes_filt[i][2]-boxes_filt[i][0])<100:
                # print(item)
                boxes_filt = boxes_filt[i].unsqueeze(0)
                pred_phrases = [pred_phrases[i]]
                # print(i)
...
if __name__ == "__main__":
    ...
    target_tasks = ['put_rubbish_in_bin','stack_blocks','put_toilet_roll_on_stand','slide_block_to_target','water_plants','sweep_to_dustpan']
    CAT_LIST.update({'bimanual_pick_laptop':'laptop','bimanual_push_box':'box','bimanual_pick_laptop':'laptop','bimanual_lift_tray':'tray','bimanual_lift_ball':'ball','bimanual_lift_ball':'ball','bimanual_lift_ball':'ball','phone_on_base':'grey phone near the red base on the desk','place_hanger_on_rack':'handle of hanger','press_switch':'switch button', 'put_knife_on_chopping_board':'knife','put_plate_in_colored_dish_rack':'white dish','take_frame_off_hanger':'frame','take_off_weighing_scales':'red pepper','take_umbrella_out_of_umbrella_stand':'umbrella_handle'})
    #'beat_the_buzz':'orange handle','change_clock':'wooden button',
    data_list = os.listdir(image_dir)
    
    for data_item in tqdm.tqdm(sorted(data_list)):
        
        category = None
        # data_item = 'put_knife_on_chopping_board-2024-10-18-7-49-0-10-85'
        if 'laptop' not in data_item:
        ...
        if category == None:
            continue
        print(category)
        sample_item = os.listdir(os.path.join(image_dir,data_item,'all_variations','episodes'))
        for sample_ins in sorted(sample_item):
        
            img_path = os.path.join(image_dir,data_item,'all_variations','episodes',sample_ins,'front_rgb',rgb_id+'.png')
            output_path = os.path.join(image_dir,data_item,'all_variations','episodes',sample_ins,'front_rgb')
            front_rgb_list = os.listdir(output_path)
            # for rgb_image_name in front_rgb_list:
            #     if '1022' in rgb_image_name:
            #         FLAG = 'True'
            #         break
