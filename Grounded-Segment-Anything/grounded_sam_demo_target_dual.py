...
def masked_image(mask, image, out_dir, rgb_id, category):
    ...
    # print(os.path.join(out_dir,rgb_id+'_{}_{}_box_target2_1022.png'.format(i,category)))
    # assert(0)
    ...
    if 'chopping' in category:
        ...
        for i,item in enumerate(pred_phrases):
            ...
            if 'chopping' in item and boxes_filt[i][0] > 20:
                ...
                # assert(0)
                break
    if 'lines' in category:
        for i,item in enumerate(pred_phrases):
            # print(i,item)
            if 'lines' in item and abs(boxes_filt[i][2]-boxes_filt[i][0]) < 20:
                # print(item)
                boxes_filt = boxes_filt[i].unsqueeze(0)
                pred_phrases = [pred_phrases[i]]
                # print(i)
...
if __name__ == "__main__":
    ...
    # TARGET_CAT_LIST = {'put_rubbish_in_bin':'black bin','stack_blocks':'the biggest green block in the center','put_toilet_roll_on_stand':'the standing stand','slide_block_to_target':'green square','water_plants':'green plant','sweep_to_dustpan':'dustpan'}
    TARGET_CAT_LIST = {'stack_blocks':'light green'}
    TARGET_CAT_LIST.update({'bimanual_straighten_rope':'green block','bimanual_push_box':'red square','put_knife_on_chopping_board':'chopping board','put_plate_in_colored_dish_rack':'red lines','straighten_rope':'green block'})
    #'phone_on_base':'orange_phone', 'place_hanger_on_rack':'grey long rack line',
    ...
    for data_item in tqdm.tqdm(sorted(data_list)):
        ...
        for sample_ins in sorted(sample_item):
            ...
            #     continue
            print(img_path)
            img_path = '/new/algo/user8/lixiaoqi/cloris-2/34714e014a5caa632762e1eb0444360c.png'
            category = 'black box. small orange block'
            output_path = '/new/algo/user8/lixiaoqi/cloris-2'
