...
def eval_seed(#train_cfg,
    ...
    '''p = Process(target=env_runner.start,
                    ...
                    int(eval_cfg.framework.gpu),
                    eval_cfg.framework.eval_save_metrics,
                    eval_cfg.cinematic_recorder,eval_cfg))
    #print(2)
    p.start()
            #processes.append(p)
        #for p in processes:
    ...
    env_runner.start(save_load_lock,
                    writer_lock,
                    env_config,
                    int(eval_cfg.framework.gpu),
                    eval_cfg.framework.eval_save_metrics,
...
def main(eval_cfg: DictConfig) -> None:
    ...
    model_name =eval_cfg.framework.adapter_path.split('/')[-2]
    ...
    if len(eval_cfg.rlbench.tasks) > 1:
        ...
        for task in tasks:
            ...
            task_classes.append(task_file_to_task_class(task))     ###任务名转换为任务类
