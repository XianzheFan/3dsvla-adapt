...
flags.DEFINE_string('save_path',
                    './RLBench/train_dataset_1223',
                    'Where to save the demos.')
flags.DEFINE_list('tasks', ['close_laptop_lid','open_microwave','place_wine_at_rack_location','beat_the_buzz','close_box','close_fridge','phone_on_base','press_switch',
'take_usb_out_of_computer','toilet_seat_down','unplug_charger','water_plants','put_rubbish_in_bin','sweep_to_dustpan','place_hanger_on_rack','take_umbrella_out_of_umbrella_stand','straighten_rope','take_off_weighing_scales','put_knife_on_chopping_board','change_clock'],
                  'The tasks to collect. If empty, all tasks are collected.')
flags.DEFINE_list('image_size', [672, 672],
...
def if_collected(i,demo, key_frames):
    if (i==len(demo)-1):
        if_coll = True
        if if_coll:
            key_frames.append(i)
            if i-1 in key_frames:
                if_coll = False
        return if_coll, key_frames
    elif i==0:
        if_coll = True
        key_frames.append(i)
        return if_coll, key_frames
    else:  
        gripper_state_no_change = (demo[i].gripper_open == demo[i-1].gripper_open and demo[i].gripper_open == demo[i+1].gripper_open)
        # small_velocity = (demo[i].joint_velocities[3]<0.1)
        small_velocity = all(abs(value) < 0.1 for value in demo[i].joint_velocities)
        if_coll = False
        if small_velocity and (i-1) not in key_frames and (i-2) not in key_frames:
            key_frames.append(i)
            if_coll = True
                
        if demo[i].gripper_open != demo[i-1].gripper_open == 0:
            key_frames.append(i)
            if_coll = True
        if np.linalg.norm(demo[i].gripper_touch_forces) > 0.01 and np.linalg.norm(demo[i-1].gripper_touch_forces) < 0.01:
            key_frames.append(i)
            if_coll = True
        return if_coll, key_frames
...
def save_demo(demo, example_path, variation):
    ...
    for i, obs in enumerate(demo):
        joint_velocities = demo[i].joint_velocities
        # print(if_collected(i,demo),demo[i].joint_velocities)
        if_coll, key_frames = if_collected(i,demo,key_frames)
        if not if_coll:
            ...
            obs.front_mask = None
            continue
        # print(i,demo[i].gripper_open)
        # if i >5: 
        #     break
        # pc = np.array(obs.front_point_cloud)
        # np.save(os.path.join(front_rgb_path,'front_point_cloud_t{}.npy'.format(str(i))), pc)
        ...
        # object_handle = obs.front_mask.mymask[0, 0]
    ...
    with open(os.path.join(example_path,'variation_number.pkl'), 'wb') as f:
        # print(variation)
    ...
    while True:
        ...
        for ex_idx in range(FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                ...
                episode_path = os.path.join(FLAGS.save_path,'{}-{}-{}-{}-{}-{}-{}-{}-{}'.format(task_env.get_name(),now.year, now.month, now.day,now.hour,now.minute, now.second,str(random.randint(0,100)),str(random.randint(0,100))))
...
def main(argv):
    ...
    # task_files = [t.replace('.py', '') for t in os.listdir(task.TASKS_PATH)
    #               if t != '__init__.py' and t.endswith('.py') and 'open' not in t and 'close' not in t]
