...
class _IndependentEnvRunner(_EnvRunner):
    ...
    def __init__(self,
            ...
            super().__init__(train_env, eval_env, #agent, 
                             timesteps,
                             train_envs, eval_envs, rollout_episodes, eval_episodes,
                             #training_iterations, 
                             eval_from_eps_number, episode_length,
                             ...
                             target_replay_ratio, #weightsdir, 
                             logdir, env_device,
    ...
    def get_task_name(self,env):
        if hasattr(env, '_task_class'):
            eval_task_name = change_case(env._task_class.__name__)
            multi_task = False
        elif hasattr(env, '_task_classes'):
            if env.active_task_id != -1:
                task_id = (env.active_task_id) % len(env._task_classes)
                eval_task_name = change_case(env._task_classes[task_id].__name__)
            else:
                eval_task_name = ''
            multi_task = True
        else:
    ...
    def _run_eval_independent(self, name: str,
                              ...
                              device_idx=0,
                              save_metrics=True,
                              cinematic_recorder_cfg=None,eval_cfg=None):

        self._name = name
        self._save_metrics = save_metrics
        #self._is_test_set = type(weight) == dict
        self._is_test_set = True
        task_idx=0
        ...
        if rec_cfg.enabled:
            ...
            cam = VisionSensor.create(rec_cfg.camera_resolution)   ##设置相机用于录制视频
        ...
        if self._save_metrics:
            ...
            writer = LogWriter(os.path.join(self._logdir,"logs"), True, True,
                               env_csv=csv_file)
        ...
        current_task_id = -1
        text_dir= os.path.join(self._logdir, 'text.txt')
        #with open(text_dir, 'w') as file:
        #    file.write('')  # 创建一个空文件
        for n_eval in range(self._num_eval_runs):       ###task种类数
            if rec_cfg.enabled:
            ...
            '''if hasattr(env, '_task_class'):
                #env._task_class.__name__.init_task(env._task_class.__name__)
                #env._task_class.init_task()
                env._task._task.init_task()
            elif hasattr(env, '_task_classes'):
                if env.active_task_id != -1:
                    task_id=(env.active_task_id) % len(env._task_classes)
                    #env._task_classes[task_id].init_task(env._task_classes[task_id])
                    #env._task_classes[task_id].init_task()
                    env._task._task.init_task()
                else:
            ...
            task_name_2=eval_cfg.rlbench.tasks[task_idx]
            #task_name=self._get_task_name()[0]
            task_dir= os.path.join(self._logdir,task_name_2)
            os.makedirs(task_dir, exist_ok=True)

            text_dir= os.path.join(self._logdir,task_name_2, 'text.txt')
            with open(text_dir, 'w') as file:
                file.write('')  # 创建一个空文件
            ...
            for ep in range(self._eval_episodes):          ###eval每个task的episode数
                eval_demo_seed = ep + self._eval_from_eps_number
                logging.info('%s: Starting episode %d, seed %d.' % (name, ep, eval_demo_seed))

                with open (text_dir, 'a') as f:
                    f.write(task_name_2+'\n')
                    f.write('Starting episode '+str(ep)+', seed '+str(eval_demo_seed)+'\n')

                #task_name_3=get_task_name_2(env)
                ...
                generator = self._rollout_generator.generator(
                    ...
                    record_enabled=rec_cfg.enabled,env_device=device,logdir=self._logdir,eval_cfg=eval_cfg,task_name=task_name_2)
                try:
                ...
                with open (text_dir, 'a') as f:
                    f.write(f'Evaluating {task_name} | Episode {ep} | Score: {reward} | Lang Goal: {lang_goal}\n')
                ...
                if rec_cfg.enabled:
                    ...
                    record_file = os.path.join(self._logdir, 'videos',
