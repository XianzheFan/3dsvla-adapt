...
front_camera_intrinsics = np.array([[-923.1524421, 0, 336],[0, -923.1524421,  336],[0,0,1]])
class NumpyCompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        # Some pickles refer to internal numpy modules like numpy._core.*
        # Map them to numpy.core.* for older numpy versions (py3.8-friendly).
...
def format_prompt_question(data_item,task_name):
    ...
    if gripper_pos_now is not None:
        ...
        if target_box_data:
            ...
            if 'rope' not in task_name and 'dish' not in task_name:
            ...
            depth_dir = os.path.join('/'.join(image_path.split('/')[:-2]),'front_depth','0.png')
            world_point = discretize_1(pixel_to_worldpoint(depth_dir, data_item,target_center_2d[1],target_center_2d[0]))

            prompt=(
                    "Determine the gripper's position, orientation, and operational state for the next step in the task: {task}. The current gripper position is [{pos_str}]. The current gripper rotation is [{rot_str}]. The current gripper state is {gripper_open_now}. The target position is within or near the point: {target_point}").format_map({'task': ' '.join(task_name.split('_')), 'pos_str': pos_str, 'rot_str': rot_str, 'gripper_open_now': gripper_open_now,'target_point':world_point})
        else:
            prompt=(
                    "Determine the gripper's position, orientation, and operational state for the next step in the task: {task}. The current gripper position is [{pos_str}]. The current gripper rotation is [{rot_str}]. The current gripper state is {gripper_open_now}").format_map({'task': ' '.join(task_name.split('_')), 'pos_str': pos_str, 'rot_str': rot_str, 'gripper_open_now': gripper_open_now})
    else:
        if box_data:
...
def format_prompt_answer(data_item,task_name):
    ...
    if len(gripper_pose_future)==6:
        pos=gripper_pose_future[:2]
        rot=gripper_pose_future[2:]
    else:
...
def get_dataset(root_path,output_path):
    ...
    ann=[]
    import tqdm
    for episode_folder in tqdm.tqdm(episode_folders):
       
        if 'close_drawer' in episode_folder or 'open_fridge' in episode_folder or 'turn_tap' in episode_folder or 'push_button' in episode_folder or 'put_toilet_roll_on_stand' in episode_folder or 'close_jar' in episode_folder or 'pour_from_cup_to_cup' in episode_folder:
            continue
        task_name = episode_folder.split('/')[-1].split('-')[0]
        for task_item in list(CAT_LIST.keys()):
            if task_name.lower() in task_item.lower() or task_item.lower() in task_name.lower():
                category = CAT_LIST[task_item]
                break
        
        image_folder = os.path.join(episode_folder, 'front_rgb')
        with open(os.path.join(episode_folder, 'low_dim_obs.pkl'), 'rb') as f:
            demo =  NumpyCompatUnpickler(f).load()
        ...
        if 'plant' in image_paths[0]:
            
            if int(demo[image_idx[-1]].gripper_open) == 1:
                for idx,item in enumerate(demo):
                    
                    if int(demo[len(demo)-idx-1].gripper_open) == 0:
                        if len(demo)-idx-1 not in image_idx:
                            image_idx[-1] = len(demo)-idx-1
                        else:
        ...
        joint_force=[demo[idx].joint_forces[6] for idx in image_idx]
        if 'press_switch' in episode_folder:
            rot_last =  quaternion_to_rotation_matrix(gripper_pos[-1][3:])
            if rot_last[:,2][0] > 0:
                
                continue
        elif 'frame' in episode_folder:
            rot_last =  quaternion_to_rotation_matrix(gripper_pos[-1][3:])
            if rot_last[:,2][1] > 0.7:
                
                continue
        image_paths, gripper_pos, gripper_open,touch_force,joint_force=idx_filter(image_paths, gripper_pos, gripper_open, touch_force,joint_force)

        image_paths, gripper_pos, gripper_open,touch_force, joint_force,bool_list=idx_touch_filter(image_paths, gripper_pos, gripper_open, touch_force, joint_force)
        if 'slide' in task_name:
            bool_list[1] = False
        if 'weighing' in task_name:
            gripper_open = [0.0] * (len(gripper_open)-1)
            gripper_open.append(1.0)
        if 'beat' in task_name:
            gripper_open = [0.0] * len(gripper_open)
        if 'hanger' in task_name and 'rack' in task_name:
            gripper_open = [0.0] * len(gripper_open)
        if 'clock' in task_name:
            gripper_open = [0.0] * (len(gripper_open)-1)
            gripper_open.append(1.0)

        assert len(image_paths)==len(gripper_pos)==len(gripper_open)==len(touch_force)==len(joint_force)==len(bool_list)
        
        assert len(gripper_pos) == len(image_paths)==len(gripper_open)

        for i in range(len(gripper_pos)):
            count = 0
            if i==len(gripper_pos)-1:
                break 
            elif i==0 or (bool_list[i] and not bool_list[i+1]):
                # print('-----2d')
                count += 1
                image_idx=int(os.path.basename(image_paths[i+1]).split('.')[0])
                box_data = None
                target_box_data = None
                y,x = worldpoint_to_pixel(demo[image_idx])
                image_list = os.listdir('/'.join(image_paths[i].split('/')[:-1]))
                for item in sorted(image_list):
                    if 'usb' in task_name:
                        
                        if 'json' in item and (category in str(item) or str(item) in category) and 'target' not in item:
                            with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                                box_data = json.load(f)
                                break
                        
                    if 'stack' in task_name:
                        box_data = {"top_left": [y-50, x-50], "bottom_right": [y+50, x+50]}
                        break
                    if 'rope' in task_name:
                        box_data = {"top_left": [y-50, x-50], "bottom_right": [y+50, x+50]}
                    if 'cup' in task_name:
                        box_data = {"top_left": [y-50, x-50], "bottom_right": [y+50, x+50]}
                    # if '0910' in item and 'json' in item and (category in str(item) or str(item) in category) and 'target' not in item:
                    #     with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                    #         box_data = json.load(f)
                    #         break
                    # if '1021' in item and 'json' in item and (category in str(item) or str(item) in category) and 'phone' not in task_name and 'weighing' not in task_name and 'knife' not in task_name and 'target' not in item:
                    #     with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                    #         box_data = json.load(f)
                    #         break
                    # if '1022' in item and 'json' in item and (category in str(item) or str(item) in category) and 'target' not in item:
                        
                    #     with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                    #         box_data = json.load(f)
                            
                    #         break
                if 'frame' in task_name and box_data != None:
                    if box_data['top_left'][0] == 0:
                        
                        break
                if 'phone' in task_name and box_data != None:
                    if box_data['top_left'][1] == 0:
                        break
                  
                
                if "drawer" not in variation_description[0] and "light" not in variation_description[0] and "fridge" not in variation_description[0] and "laptop" not in variation_description[0]:
                    
                    if isinstance(x, np.int64):
                        x = int(x)
                    if isinstance(y, np.int64):
                        y = int(y)
                pos=[x,y]+gripper_pos[i+1].tolist()[3:]
                target_box_data = None
                for item in sorted(image_list):
                    if 'target_' in item and 'json' in item :
                        if 'stand' in task_name and 'thin' in item and 'head' not in item:
                            with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                                target_box_data = json.load(f)
                                # assert(0)
                                break
                        elif 'blocks' in task_name:
                            if 'target_' in item:
                                with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                                    target_box_data = json.load(f)
                                    # assert(0)
                ...
                draw.ellipse((y - 3, x - 3, 
                            y + 3, x + 3), fill='red')
                rgb_img.save(image_paths[i].replace('.png','_draw3.png'))
                data_item = {'image': image_paths[i], 'gripper_pos_n': pos, 'gripper_open_n': gripper_open[i+1], 'variation_description': variation_description[0], 'gripper_pos_n-1': None, 'gripper_open_n-1': None,'box_data':box_data,'target_box_data':target_box_data}
            else:
                # print('-----3d')
                target_box_data = None
                target_json = 0
                for item in sorted(image_list):
                    if 'buzz' in task_name:
                        
                        if '1021' in item and 'json' in item:
                    ...
                    if ('target_' in item or 'target_' in item) and 'json' in item :
                        if 'stand' in task_name and 'thin' in item and 'head' not in item:
                            with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                                target_box_data = json.load(f)
                    ...
                    if 'target_' in item and 'json' in item:
                        ...
                        if 'dish' in task_name:
                            if 'red lines' in item:
                            ...
                            with open(os.path.join(episode_folder, 'front_rgb',item), 'rb') as f:
                                target_box_data = json.load(f)
                                # print(y,x,target_box_data)
                                center_x = (target_box_data['top_left'][0] + target_box_data['bottom_right'][0]) // 2
                                center_y = (target_box_data['top_left'][1] + target_box_data['bottom_right'][1]) // 2
                                if abs(y-center_x) < 70:
                                    
                                    # print('1111111111',item)
                                    break
                                else:
                                    continue
                ...
                data_item = {'image': image_paths[i], 'gripper_pos_n': gripper_pos[i+1].tolist(), 'gripper_open_n': gripper_open[i+1], 'variation_description': variation_description[0], 'gripper_pos_n-1': gripper_pos[i].tolist(), 'gripper_open_n-1': gripper_open[i],'box_data':None,'target_box_data':target_box_data}
...
if __name__ == '__main__':
    ...
    ann = get_dataset(root_path,output_path)
